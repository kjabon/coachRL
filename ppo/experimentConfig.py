## This file is altered from the acme/jax/experiments/experimentConfig.py
## The only change is to "unfreeze" the config class to allow parameter setting.
...
from acme.agents.jax.builders import Policy, Networks, Sample, ActorLearnerBuilder, OfflineBuilder
...
class MakeActorFn(Protocol, Generic[Policy]):

  def __call__(self, random_key: types.PRNGKey, policy: Policy,
               environment_spec: specs.EnvironmentSpec,
               variable_source: core.VariableSource) -> core.Actor:
    ...


class NetworkFactory(Protocol, Generic[Networks]):

  def __call__(self,
               environment_spec: specs.EnvironmentSpec) -> Networks:
    ...


class DeprecatedPolicyFactory(Protocol, Generic[Networks,
                                                Policy]):

  def __call__(self, networks: Networks) -> Policy:
    ...


class PolicyFactory(Protocol, Generic[Networks, Policy]):

  def __call__(self, networks: Networks,
               environment_spec: specs.EnvironmentSpec,
               evaluation: bool) -> Policy:
    ...


class EvaluatorFactory(Protocol, Generic[Policy]):

  def __call__(self, random_key: types.PRNGKey,
               variable_source: core.VariableSource, counter: counting.Counter,
               make_actor_fn: MakeActorFn[Policy]) -> core.Worker:
    ...


class SnapshotModelFactory(Protocol, Generic[Networks]):

  def __call__(
      self, networks: Networks, environment_spec: specs.EnvironmentSpec
...
class ExperimentConfig(Generic[Networks, Policy,
                               Sample]):
  ...
  builder: ActorLearnerBuilder[Networks, Policy,
                                        Sample]
  network_factory: NetworkFactory[Networks]
  ...
  def get_evaluator_factories(self):
    ...
    def eval_policy_factory(networks: Networks,
                            environment_spec: specs.EnvironmentSpec,
                            evaluation: bool) -> Policy:
      del evaluation
...
class OfflineExperimentConfig(Generic[Networks, Policy,
                                      Sample]):
  ...
  """
  # Below fields must be explicitly specified for any Agent.
  builder: OfflineBuilder[Networks, Policy,
                                   Sample]
  network_factory: Callable[[specs.EnvironmentSpec], Networks]
...
def default_evaluator_factory(
    environment_factory: types.EnvironmentFactory,
    network_factory: NetworkFactory[Networks],
    policy_factory: PolicyFactory[Networks, Policy],
    logger_factory: loggers.LoggerFactory,
    observers: Sequence[observers_lib.EnvLoopObserver] = (),
) -> EvaluatorFactory[Policy]:
...
def make_policy(experiment: ExperimentConfig[Networks, Policy,
                                             Any], networks: Networks,
                environment_spec: specs.EnvironmentSpec,
                evaluation: bool) -> Policy:
