# Adapted from ppo networks file to support MultiDiscrete action distributions
# Search MultiDiscrete or multicategorical to dig into the details.
...
def make_networks(
    ...
    if isinstance(spec.actions, specs.DiscreteArray) or 'int' in spec.actions.dtype.name:
...
def make_mvn_diag_ppo_networks(
    ...
    def entropy(params: MVNDiagParams):
        return tfd.MultivariateNormalDiag(
...
def make_tanh_normal_ppo_networks(
    ...
    def entropy(params: TanhNormalParams):
        distribution = build_distribution(params)
        return distribution.entropy(seed=jax.random.PRNGKey(42))
...
def make_discrete_networks(
    ...
    # Assuming "MultiDiscrete"
    action_dims = environment_spec.actions.maximum
    action_dims = [x + 1 for x in action_dims]
    num_logits = sum(action_dims)

    def forward_fn(inputs):
        layers = []
        # if use_conv:
        #     layers.extend([networks_lib.AtariTorso()])
        layers.extend([hk.nets.MLP(hidden_layer_sizes, activate_final=True)])  # This should be fine for now
        trunk = hk.Sequential(layers)
        h = utils.batch_concat(inputs)
        h = trunk(h)

        logits = hk.Linear(num_logits)(h)
    ...
    return make_multicategorical_ppo_networks(network, np.array(action_dims))  # pylint:disable=undefined-variable


def make_multicategorical_ppo_networks(network: networks_lib.FeedForwardNetwork, action_dims) -> PPONetworks:
    """Constructs a PPONetworks for Multi-Categorical Policy from FeedForwardNetwork.
    #Kudos to stable-baselines3
    Args:
      network: a transformed Haiku network (or equivalent in other libraries) that
        takes in observations and returns the action distribution and value.
    ...
    def jax_unstack(x, axis=0):
        unstacked = [lax.index_in_dim(x, i, axis, keepdims=False) for i in range(x.shape[axis])]
        return unstacked

    def get_distribution(params):
        split_indices = list(accumulate(action_dims[:-1]))
        distribution = [tfd.Categorical(logits=split) for split in jnp.split(params.logits, split_indices, axis=1)]
        return distribution
    ...
    def entropy(params):
        ent = jnp.stack([dist.entropy() for dist in get_distribution(params)], axis=1).sum(axis=1)
        return ent
    ...
    def sample_eval(params, key):
        del key
        samp = jnp.stack([dist.mode() for dist in get_distribution(params)], axis=1)
        return samp
